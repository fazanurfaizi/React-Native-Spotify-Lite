...
import { AllCategoriesResponse, Category } from '@/models/AllCategoriesResponse'
...
import {
    ...
    Track,
    Tracks
} from '@/models/SpotifyCommon'
import SpotifyEndpoints, { SpotifyAPI } from './SpotifyEndpoints'
import { CurrentPlayingTrack } from '@/models/CurrentPlayingTrack'
...
class SpotifyApiService implements SpotifyAPI {
    ...
    private doThrow = async (res: Response, contentType: ContentType) => {
        ...
        throw new SpotifyHttpException(res.status, "Unsupported Content Type", res.url)
    }
    ...
    sessionIsExpired = (err: any) => {
        if(err instanceof SpotifyHttpException && err.status == 401) {
            if(err.message === "Invalid access token" || err.message.includes("expired")) {
                return true;
            }
        }
    }

    getMyProfile = () => from(this.api<ProfileResponse>({ url: SpotifyEndpoints.getMyProfile() }))
