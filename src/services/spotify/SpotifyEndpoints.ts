...
class SpotifyEndpoints implements SpotifyAPI {    
    ...
    private loginConfig = {
        clientId: "98938804e06648f3a535bbc868c673ad",
        responseType: "token",
    ...
    getRecentlyPlayedTracks = () => `${this.V1}/me/player/recently-played?limit=20`
    ...
    getSavedStateForAlbums = (ids: string) => `${this.V1}/me/albums/contains?ids=${ids}`
    
    getFollowingStateForArtistsOrUsers = (ids: string, type: "artist" | "user") => `${this.V1}/me/following/contains?type=${type}&ids=${ids}`
    
    getFollowingStateForPlaylist = (id: string, userId: string) => `${this.V1}/playlists/${id}/followers/contains?ids=${userId}`
    
    getSavedStateForTracks = (ids: string) => `${this.V1}/me/tracks/contains?ids=${ids}`
    
    saveAlbums = (ids: string) => `${this.V1}/me/albums?ids=${ids}`
    
    followArtistsOrUsers = (ids: string, type: "artist" | "user") => `${this.V1}/me/following?type=${type}&ids=${ids}`
    ...
    getAllCategoriesForCountry = (country: string) => `${this.V1}/browse/categories?country=${country}`
    
    getCategory = (id: string, urlQuery: string) => `${this.V1}/browse/categories/${id}/playlists?${urlQuery}`
    ...
    getArtistTopTracks = (id: string, country: string) => `${this.V1}/artists/${id}/top-tracks?market=${country}`
    
    search = (query: string) => `${this.V1}/search?market=MY&type=album,artist,playlist,track&q=${query}`
}
