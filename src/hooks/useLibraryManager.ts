...
import { SpotifyErrorResponse } from '@/models/SpotifyCommon'
...
export const SpotifyLibraryManager = () => {
    ...
    const getSavedStateForTracks = async (formattedIds: string) => {
        try {
            const [isSaved] = await SpotifyApiService.getSavedStateForTracks(formattedIds)
            return isSaved
        } catch (error) {
            handleError(error)
        }
    ...
    const getSavedStateForAlbums = async (formattedIds: string) => {
        try {
            const [isSaved] = await SpotifyApiService.getSavedStateForAlbums(formattedIds)
    ...
    const followArtistsOrUsers = async (formattedIds: string, type: "artist" | "user") => {
        try {
            const res = await SpotifyApiService.followArtistsOrUsers(formattedIds, type)
            if(!res.ok) {
                const err: SpotifyErrorResponse = await res.json()
                throw err
            }
    ...
    const getFollowingStateForArtistsOrUsers = async (formattedIds: string, type: "artist" | "user") => {
        try {
            const [isSaved] = await SpotifyApiService.getFollowingStateForArtistsOrUsers(formattedIds, type)
    ...
    const getFollowingStateForPlaylist = async (id: string, userId: string) => {
        try {
            const [isSaved] = await SpotifyApiService.getFollowingStateForPlaylist(id, userId)
        ...
        switch (type) {
            ...
            case "PLAYLIST":
                if(typeof userId === "string") {
                    ...
                    throw new Error("getItemSavedOrFollowedState: User ID is not a string")
                }
            case "TRACK":
