...
const getCurrentUserSavedTracksEpic = (action$: Observable<Action<any>>) =>
    action$.pipe(
        ...
        switchMap(() => 
            SpotifyApiService.getCurrentUserSavedTracks().pipe(
                map((res) => {
                    ...
                    return getCurrentUserSavedAlbumsSuccess({
                        count: res.total,
                        data
                    })
                }),
...
const getCurrentUserSavedAlbumsEpic = (action$: Observable<Action<any>>) =>
    action$.pipe(
