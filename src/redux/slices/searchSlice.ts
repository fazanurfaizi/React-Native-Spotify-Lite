...
const searchSlice = createSlice({
    ...
    reducers: {
        ...
        querySave: (state, action) => {
            if(
                state.queryHistory.findIndex(item => item.id === action.payload.id) === -1
            ) {
                return {
                    ...state,
                    queryHistory: [
...
const searchForQueryEpic = (action$: Observable<Action<string>>) => 
    action$.pipe(
        ...
        switchMap(action => {
            ...
            const request$ = SpotifyApiService.search(query).pipe(
                map(data => {
                    ...
                    const results = prepareSearchResult(data)
...
export const setSeeAll = (data: AlbumType[]) => {
    switch (data[0].type) {
        ...
        case "Artist":
            return querySetSeeAll({ data, type: "Artist" })              
        case "Playlist":
            return querySetSeeAll({ data, type: "Playlist" })              
        case "Song":
            return querySetSeeAll({ data, type: "Song" })              
        default:
    ...
    for(let i = 0; i < ATTEMPTS; i++) {
        ...
        if(
            ...
            const mostPopular = randomArray.reduce((prev, current) => {
                const notPreviouslyAdded = topSearchResults.findIndex(item => item.id === current.id) === -1
            ...
            const notPreviouslyAdded = topSearchResults.findIndex(item => item.id === mostPopular.id) === -1
            ...
            for (let j = 0; j < randomArray.length; j++) {
                ...
                const notPreviouslyAdded = topSearchResults.findIndex(item => item.id === randomItem.id) === -1
...
const sortBySongAndArtistFirst = (topSearchResults: AlbumType[]) => {
    return topSearchResults.sort((firstItem, secondItem) => {
        const firstItemIsArtistOrSong = firstItem.type === "Song" || firstItem.type === "Artist"
        const secondItemIsArtistOrSong = secondItem.type === "Song" || secondItem.type === "Artist"
